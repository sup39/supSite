 ...
 * @param {string} dirRPath -- file directory (relative)
 */
const getFileCompileInfo = (src, dirRPath='') => {
  for (const {test, output, compile} of options.rules) {
    if (test.test(src)) {
      const srcParsed = path.parse(src);
      const dst = path.join(dirRPath, output
...
const listDir = async dirRPath => {
  ...
  const list2 = await Promise.all(info.nav.flatMap(async ({title, src: srcName}) => {
    ...
    if (src.endsWith('/')) {
      ...
      const info = getFileCompileInfo(src, dirRPath);
      if (info == null) return [];
      return /**@type{NavItem}*/({type: 'src', title, ...info});
    }
  }));
  return list2.flatMap(e => e)
    .concat(info.static?.map(src0 => {
      const src = path.join(dirRPath, src0);
      return {type: 'static', src, dst: src};
    }) ?? []);
};
...
function makeNavTag(info) {
  if (info.type === 'dir') {
    ...
    return `<div class="${navCss.dirCtn}"><div class="${navCss.dirRow}"><a href="${href}">${title}</a>${svgFolder}</div><div class="${navCss.dirChild}">${info.children.map(makeNavTag).join('')}</div></div>`;
...
const {dst: indexDstName, compile: indexCompiler} = getFileCompileInfo(indexSrcName) ?? {
  dst: indexSrcName,
  compile: (src, dst) => fs.cp(src, dst, {recursive: true}),
};
/**
 * @param {NavItem} item
 * @param {string} nav
...
async function compileFile(item, nav) {
  ...
  if (type === 'dir') {
    ...
    printDebug(path.join(item.src, indexSrcName), '-(compile)>', path.join(item.dst, indexDstName));
    // compile children
    await Promise.all(item.children.map(c => compileFile(c, nav)));
  } else if (type === 'src') {
    await item.compile(srcAPath, dstAPath, nav);
    printDebug(item.src, '-(compile)>', item.dst);
  } else if (type === 'static') {
    await fs.cp(srcAPath, dstAPath, {recursive: true});
    printDebug(item.src, '-(copy)>', item.dst);
  }
}
...
listDir('/').then(async items => {
  ...
  const nav = `<nav>${svg}${home}<div>${items.map(makeNavTag)}</div></nav>`;
  // clear files
  await fs.readdir(dstRoot).then(
    fns => Promise.all(fns.map(fn => fs.rm(fn, {recursive: true, force: true}))), ()=>[]);
  // compile files
